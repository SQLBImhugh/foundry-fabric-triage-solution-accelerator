"""Shared monitoring rules over explicit semantic and record interfaces.

Adapter-specific transitions have no default implementation. Both adapters
must supply their native receipt, projection and guarded-mutation semantics.
...
logger = logging.getLogger("triage.monitoring.engine")
...
class MonitoringEngine:
    def __init__(
        ...
        adapter_factory: Callable[[MonitoringEngine], MonitoringAdapter],
        ...
        self._adapter = adapter_factory(self)
    ...
    def _run_operation(self, method: Callable[..., ResultT], write: bool, args: tuple[object, ...], kwargs: dict[str, object]) -> ResultT:
        return self._adapter.run_operation(method, write, args, kwargs)
    ...
    def _decode(self, record: StoredRecord, model: type[ModelT]) -> ModelT:
        return self._adapter.decode(record, model)

    def _decode_record(self, record: StoredRecord, model: type[ModelT]) -> ModelT:
        try:
    ...
    def _put(self, kind: str, key: str, context: m.MonitoringContext, value: ModelT, **indices: object) -> ModelT:
        return self._adapter.put(kind, key, context, value, **indices)
    ...
    def _idempotent(self, operation: str, request_id: str, context: m.MonitoringContext, request: BaseModel | dict[str, object], model: type[ModelT], apply: Callable[[], ModelT]) -> ModelT:
        return self._adapter.idempotent(operation, request_id, context, request, model, apply)

    def _receipt(self, operation: str, request_id: str, context: m.MonitoringContext, model: type[ModelT]) -> ModelT | None:
        return self._adapter.receipt(operation, request_id, context, model)
    ...
    def _request_reconciliation(self, control: m.DeploymentControl, *, request_id: str, topic: str, reference_id: str, fingerprint: str, payload: dict[str, object], target: m.TargetIdentity | None=None, window: m.ObservationWindow | None=None, evidence: tuple[m.EvidenceBinding, ...]=(), producer_commit: m.CollectionCommit | None=None) -> m.MonitoringWork:
        return self._adapter.request_reconciliation(control, request_id=request_id, topic=topic, reference_id=reference_id, fingerprint=fingerprint, payload=payload, target=target, window=window, evidence=evidence, producer_commit=producer_commit)

    def _pending_validation(self, identity: m.TargetIdentity) -> bool:
        return self._adapter.pending_validation(identity)

    def _pending_frontier_count(self, context: m.MonitoringContext) -> int:
        return self._adapter.pending_frontier_count(context)
    ...
    def _commit_scope_intent(self, control: m.DeploymentControl, definition: m.ScopeDefinition) -> tuple[m.DeploymentControl, m.ScopePolicy]:
        return self._adapter.commit_scope_intent(control, definition)
    ...
    def _delivery_candidates(self, context: m.MonitoringContext, connector: m.OwnedConnectorManifest, collector_identity_id: str) -> list[m.SignalReceipt]:
        return self._adapter.delivery_candidates(context, connector, collector_identity_id)

    def _delivery_original(self, signal: m.SignalReceipt, control: m.DeploymentControl) -> datetime:
        return self._adapter.delivery_original(signal, control)
    ...
    def _connector_observation_projection(self, context: m.MonitoringContext, request_id: str) -> m.ConnectorObservationResult | None:
        return self._adapter.connector_observation_projection(context, request_id)
    ...
    def _connector_publication(self, request: m.ConnectorPublicationRequest) -> m.ConnectorPublicationResult:
        return self._adapter.connector_publication(request)
    ...
    def _reconcile_connector_observation(self, producer: m.ReconciliationRequest, connector: m.OwnedConnectorManifest, control: m.DeploymentControl) -> Rejection | None:
        return self._adapter.reconcile_connector_observation(producer, connector, control)
    ...
    def _connector_observation_overtaken(self, producer: m.ReconciliationRequest, connector: m.OwnedConnectorManifest, control: m.DeploymentControl) -> bool:
        return self._adapter.connector_observation_overtaken(producer, connector, control)
    ...
    def _commit_review_intent(self, control: m.DeploymentControl, pending: m.SafetyReview, expected_review_revision: int) -> tuple[m.DeploymentControl, m.SafetyReview]:
        return self._adapter.commit_review_intent(control, pending, expected_review_revision)
    ...
    def _save_work(self, work: m.MonitoringWork) -> m.MonitoringWork:
        return self._adapter.save_work(work)
    ...
    def _schedule_verification(self, action: m.ActionReservation) -> m.MonitoringWork:
        return self._adapter.schedule_verification(action)
    ...
    def _source_disposition(self, execution: m.SourceExecutionIdentity, disposition: str, detail: str, *, work_id: str | None=None, finalization_id: str | None=None, incident_identity: m.IncidentIdentity | None=None, observation: m.SourceRunObservation | None=None) -> m.ProcessedSourceRecord:
        return self._adapter.source_disposition(execution, disposition, detail, work_id=work_id, finalization_id=finalization_id, incident_identity=incident_identity, observation=observation)
    ...
    def _save_source(self, observation: m.SourceRunObservation) -> m.SourceRunObservation:
        return self._adapter.save_source(observation)
    ...
    def _submitted_action_owner(self, execution: m.SourceExecutionIdentity) -> ActionOwner | None:
        return self._adapter.submitted_action_owner(execution)
    ...
    def _finish_poll_work(self, work: m.MonitoringWork) -> m.MonitoringWork:
        return self._adapter.finish_poll_work(work)
    ...
    def _action_commit_work(self, context: m.MonitoringContext, commit: m.CollectionCommit) -> m.MonitoringWork:
        return self._adapter.action_commit_work(context, commit)
